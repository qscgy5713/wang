...
class CommonMemberPrivateatmModel extends CommonModel {
     ...
     * 批量修改專用
     */
    public function batchEditPrivateatmData($data){
        $sql = "UPDATE `common_member_privateatm` SET `privateATM_bankaccount` = '{$data['afterBankaccount']}', `privateATM_bankname` = '{$data['afterBankname']}'
                WHERE `privateATM_bankaccount` = '{$data['beforeBankaccount']}' AND `privateATM_bankname` = '{$data['beforeBankname']}'";
        $return = $this->query($sql);
        return $return;
    }
}