...
class CommonAgentDomainModel extends CommonModel {
    ...
    public function getDomainDateNumber($data){
        ...
        $sql = "SELECT * FROM `common_agent_domain`
        ORDER BY `domain_order` ASC {$pageSql};";
        $return = $this->query($sql);
        return $return;
    }
    /**
     * 排序
     */
    public function sortDomainDateOrder($data){
        $sql = "UPDATE `common_agent_domain` a, `common_agent_domain` b
        SET a.`domain_order` = '{$data['domain_order2']}',
            b.`domain_order` = '{$data['domain_order']}'
        WHERE a.`domain_order` = '{$data['domain_order']}'
        AND b.`domain_order` = '{$data['domain_order2']}'";
        $this->query($sql);
    }
}