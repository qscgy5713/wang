...
<div id="page-container" class="fade page-sidebar-fixed page-header-fixed page-with-wide-sidebar">
    ...
    <div id="content" class="content">
        <form id="formInquire" method="post" action='__APP__/<?php echo (MODULE_NAME); ?>/<?php echo (ACTION_NAME); ?>'>
            <input type="hidden" id="pageNumber" name="pageNumber" value="<?php echo ($pageNumber); ?>">
        </form>
        <p>
            <button id="add_btn" class="btn btn-info collapsed btn-style" type="button" data-toggle="collapse" data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">新增訊息預設</button>
        </p>
        <div class="modal fade add-page" id="modal-dialog" style="display: none;">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">新增訊息預設</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <form method="post" action='__APP__/NewsletterNotification/addFormNewsletterData'>
                        <div class="modal-body">
                            <div class="form-group row m-b-15">
                                <label class="col-md-4 col-sm-4 col-form-label">訊息識別碼 :</label>
                                <div class="col-md-8 col-sm-8">
                                    <input class="form-control" type="text" name="addMessageCode">
                                </div>
                            </div>
                            <div class="form-group row m-b-15">
                                <label class="col-md-4 col-sm-4 col-form-label">訊息內容 :</label>
                                <div class="col-md-8 col-sm-8">
                                    <textarea class="form-control" name="addMessageMain"></textarea>
                                </div>
                            </div>
                            ...
                            <table class="table table-striped m-b-0">
                                ...
                                <tbody>
                                    <?php if(is_array($showArray)): $i = 0; $__LIST__ = $showArray;if( count($__LIST__)==0 ) : echo "$showEmpty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                                            <td><?php echo ($vo["messageCode"]); ?></td>
                                            <td><?php echo ($vo["messageMain"]); ?></td>
                                            <td><?php echo ($vo["createTime"]); ?></td>
                                            <td><?php echo ($vo["modifyTime"]); ?></td>
                                            <td>
                                                <a onclick="openmodal('setModal','<?php echo ($vo["messageCode"]); ?>','<?php echo ($vo["messageMain"]); ?>')"><i class="fas fa-wrench fa-fw icon-size"></i></a>
                                                <form id="delform<?php echo ($vo["messageCode"]); ?>" class="div-style" method="post" action='__APP__/NewsletterNotification/delFormNewsletterData'>
                                                    <input type="hidden" value="<?php echo ($vo["messageCode"]); ?>" name="delMessageCode">
                                                    <a onclick="submit('delform<?php echo ($vo["messageCode"]); ?>','<?php echo ($vo["messageCode"]); ?>')"><i class="fas fa-trash fa-fw icon-size"></i></a>
                                                </form>
                                            </td>
        ...
        <div class="modal fade" id="setModal" style="display: none;">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title"> 修改訊息訊息</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <form method="post" action='__APP__/NewsletterNotification/setFormNewsletterData'>
                        <div class="modal-body">
                            <div class="form-group row m-b-15">
                                <label class="col-md-4 col-sm-4 col-form-label">訊息識別碼 :</label>
                                <div class="col-md-8 col-sm-8">
                                    <input class="form-control" type="text" value="" name="setmessageCode" id="setmessageCode">
                                    <input type="hidden" value="" name="setmessageCode1" id="setmessageCode1">
                                </div>
                            </div>
                            <div class="form-group row m-b-15">
                                <label class="col-md-4 col-sm-4 col-form-label">訊息內容 :</label>
                                <div class="col-md-8 col-sm-8">
                                    <textarea class="form-control" type="text" value="" name="setmessageMain" id="setmessageMain"></textarea>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
...
function openmodal(modal_id, vo_code, vo_main) {
    $('#' + modal_id).modal('show');
    $('#setmessageMain').val(vo_main);
    $('#setmessageCode').val(vo_code);
    $('#setmessageCode1').val(vo_code);
}
$(document).ready(function() {
...
function submit(formId, vo_class) {
    askMsgShow('是否刪除 訊息識別碼"' + vo_class + '" 的訊息');
