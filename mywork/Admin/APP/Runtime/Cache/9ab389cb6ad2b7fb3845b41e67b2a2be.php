...
<div id="page-container" class="fade page-sidebar-fixed page-header-fixed page-with-wide-sidebar">
	...
	<div id="content" class="content">
		...
		<form class="m-b-10" id="formInquire" method="post" action="__APP__/<?php echo (MODULE_NAME); ?>/index">
			<div class="row">
							...
							<table id="table1" class="table table-striped m-b-0">
								...
								<tbody>
									<?php if(is_array($showArray)): $i = 0; $__LIST__ = $showArray;if( count($__LIST__)==0 ) : echo "$showEmpty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
											...
											<td>
												<a onclick="openmodal('setModal','<?php echo ($vo["memberAccount"]); ?>','<?php echo ($vo["id"]); ?>','<?php echo ($vo["code"]); ?>','<?php echo ($vo["account"]); ?>','<?php echo ($vo["name"]); ?>','<?php echo ($vo["branch"]); ?>','<?php echo ($vo["remark"]); ?>','<?php echo ($vo["bankName"]); ?>','<?php echo ($vo["memberId"]); ?>')"><i class="fas fa-wrench fa-fw icon-size"></i></a>
												<form id="delform<?php echo ($vo["id"]); ?>" class="div-style" method="post" action='__APP__/ManageBank/formDelMemberBank'>
													<input type="hidden" value="<?php echo ($vo["id"]); ?>" name="delId">
													<input type="hidden" value="<?php echo ($vo["memberAccount"]); ?>" name="delAccount">
													<input type="hidden" value="<?php echo ($vo["memberId"]); ?>" name="delMemberId">
													<input type="hidden" value="<?php echo ($vo["id"]); ?>" name="delId">
													<a onclick="submit('delform<?php echo ($vo["id"]); ?>','<?php echo ($vo["memberAccount"]); ?>')"><i class="fas fa-trash fa-fw icon-size"></i></a>
												</form>
				                            </td>
							 ...
							 <div class="form-group row m-b-15">
								...
								<div class="col-md-8 col-sm-8">
									<select class="form-control" id="setCode" name="setCode">
										<?php if(is_array($bankArray)): $i = 0; $__LIST__ = $bankArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ba): $mod = ($i % 2 );++$i;?><option value="<?php echo ($ba["bankCode"]); ?>"><?php echo ($ba["bankCode"]); echo ($ba["bankName"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select>
								</div>
							 </div>
