                            ...
                            <div class="form-group row m-b-15">
                                ...
                                <div class="col-md-8 col-sm-8">
                                    <select class="form-control" name="addClassId">
                                        <?php if(is_array($classArray)): $i = 0; $__LIST__ = $classArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["classId"]); ?>"><?php echo ($vo["showName"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group row m-b-15">
        ...
        <div class="row">
            <div class="col-lg-12 ui-sortable">
                <ul class="nav nav-tabs nav-tabs-inverse">
                    <?php if(is_array($showArray)): $k1 = 0; $__LIST__ = $showArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k1 % 2 );++$k1;?><li class="nav-item">
                            <a id="<?php echo ($key); ?>tab" href="#default-tab-<?php echo ($key); ?>" data-toggle="tab" class="nav-link">
                ...
                <div class="tab-content">
                    <?php if(is_array($showArray)): $k1 = 0; $__LIST__ = $showArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k1 % 2 );++$k1;?><div class="tab-pane fade" id="default-tab-<?php echo ($key); ?>">
                            <div class="table-responsive">
                                <table class="table table-striped m-b-0">
                                    ...
                                    <tbody>
                                        <?php if(is_array($vo)): $k2 = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "$showEmpty" ;else: foreach($__LIST__ as $key=>$vi): $mod = ($k2 % 2 );++$k2;?><tr>
                                                <td><?php echo ($k2); ?></td>
                                                <td><?php echo ($vi["classShowName"]); ?></td>
                                                <td><?php echo ($vi["actionName"]); ?></td>
                                                <td><?php echo ($vi["showName"]); ?></td>
                                                <td>
                                                    <?php if($vi["classShowName"] == $classArray[$k1-1]['showName']): if($vi["order"] > 1): ?><form id="sortup<?php echo ($vi["id"]); ?>" class="div-style" method="post" action="__APP__/AdminPageManage/formSortAgentPageClass">
                                                                <input type="hidden" value="upSort" name="className">
                                                                <input type="hidden" value="<?php echo ($vi["id"]); ?>" name="sortid">
                                                                <input type="hidden" value="<?php echo ($vi["classId"]); ?>" name="sortId">
                                                                <input type="hidden" value="<?php echo ($vi["order"]); ?>" name="orderSort">
                                                                <a onclick="submit('sortup<?php echo ($vi["id"]); ?>')" href="#">
                                                    ...
                                                    <span class="f-s-17"><?php echo ($vi["order"]); ?></span>
                                                    <?php if($vi["classShowName"] == $classArray[$k1-1]['showName']): if($vi["order"] < $classArray[$k1-1]['total']): ?><form id="sortdown<?php echo ($vi["id"]); ?>" class="div-style" method="post" action="__APP__/AdminPageManage/formSortAgentPageClass">
                                                                <input type="hidden" value="downSort" name="className">
                                                                <input type="hidden" value="<?php echo ($vi["id"]); ?>" name="sortid">
                                                                <input type="hidden" value="<?php echo ($vi["classId"]); ?>" name="sortId">
                                                                <input type="hidden" value="<?php echo ($vi["order"]); ?>" name="orderSort">
                                                                <a onclick="submit('sortdown<?php echo ($vi["id"]); ?>')" href="#">
                                                                    <i class="fas fa-arrow-alt-circle-right fa-rotate-90 sort-icon"></i></a>
                                                            </form><?php endif; endif; ?>
                                                </td>
                                                <td id="<?php echo ($vi["id"]); ?>enabled"><?php echo ($vi["enabled"]); ?></td>
                                                <td><?php echo ($vi["createtime"]); ?></td>
                                                <td><?php echo ($vi["modifyTime"]); ?></td>
                                                <td>
                                                    <a onclick="openmodal('setModal','<?php echo ($vi["id"]); ?>','<?php echo ($vi["classId"]); ?>','<?php echo ($vi["actionName"]); ?>','<?php echo ($vi["showName"]); ?>')"><i class="fas fa-wrench fa-fw icon-size"></i></a>
                                                    <?php if($vi["enabled"] == '啟用中'): ?><form id="formInquireTostop<?php echo ($vi["id"]); ?>" class="div-style form-vertical-align" method="post" action='__APP__/AdminPageManage/formStopAdminPage'>
                                                            <input type="hidden" value="<?php echo ($vi["id"]); ?>" name="stopId">
                                                            <div class="switcher">
                                                                <input type="checkbox" id="switcher_checkbox_<?php echo ($vi["id"]); ?>" checked onclick="submitAjax('formInquireTostop','<?php echo ($vi["id"]); ?>','stopId')">
                                                                <label for="switcher_checkbox_<?php echo ($vi["id"]); ?>" class="switchbtn"></label>
                                                            </div>
                                                        </form>
                                                        <?php else: ?>
                                                        <form id="formInquireTostart<?php echo ($vi["id"]); ?>" class="div-style form-vertical-align" method="post" action='__APP__/AdminPageManage/formStartAdminPage'>
                                                            <input type="hidden" value="<?php echo ($vi["id"]); ?>" name="startId">
                                                            <div class="switcher">
                                                                <input type="checkbox" id="switcher_checkbox_<?php echo ($vi["id"]); ?>" onclick="submitAjax('formInquireTostart','<?php echo ($vi["id"]); ?>','startId')">
                                                                <label for="switcher_checkbox_<?php echo ($vi["id"]); ?>" class="switchbtn"></label>
                                                            </div>
                                                        </form><?php endif; ?>
                                                    <form id="delform<?php echo ($vi["id"]); ?>" class="div-style" method="post" action='__APP__/AdminPageManage/formDelAdminPage'>
                                                        <input type="hidden" value="<?php echo ($vi["id"]); ?>" name="delId">
                                                        <a onclick="submitDel('delform<?php echo ($vi["id"]); ?>','<?php echo ($vi["showName"]); ?>')"><i class="fas fa-trash fa-fw icon-size"></i></a>
                                                    </form>
                                                </td>
                            ...
                            <div class="form-group row m-b-15">
                                ...
                                <div class="col-md-8 col-sm-8 m-t-10">
                                    <select class="form-control" name="setClassId" id="setClassId">
                                        <?php if(is_array($classArray)): $i = 0; $__LIST__ = $classArray;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ci): $mod = ($i % 2 );++$i;?><option value="<?php echo ($ci["classId"]); ?>"><?php echo ($ci["showName"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="form-group row m-b-15">
